...
class Addorder
{
    ...
    public function __construct($payment_intent , $sentObj)
    {
        $objDb = new DbConnect();
        $this->conn = $objDb->connect();//database connection
         if (!empty($payment_intent) && !empty($sentObj)) {
            $this->userid= $sentObj['userid'];
            $this->customer_name = $sentObj['name'];
            $this->customer_email = $sentObj['email'];
            $this->address= $sentObj['address'];
            $this->description= $sentObj['description'];
            $this->products= $sentObj['products']['products'];
            $this->amount = $sentObj['amount'];
            $this->centsamount = $payment_intent->amount;
            $this->paid_amount = $this->centsamount / 100;
            $this->currency = $payment_intent->currency;
            $this->transaction_id = $payment_intent->id;
